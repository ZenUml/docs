...
The rendered PNG backup used for export is uploaded to Confluence — sometimes directly from your browser, sometimes relayed through ZenUML's Cloudflare backend on the way. Either way it is not stored there; the backend forwards it to Confluence and returns. Otherwise, ZenUML's Cloudflare backend handles only app lifecycle events (install/upgrade), licence status checks, and anonymous usage telemetry.
