...
Your diagram text and rendered output is **stored as Confluence custom content on Atlassian's infrastructure** — the same servers that store your page content. Your diagram source is not sent to ZenUML servers for rendering or persistent storage.

The rendered PNG backup used for export is normally uploaded straight from your browser to Confluence. As an uncommon reliability fallback — when the viewing user's own Confluence permissions block that upload — the PNG is relayed through ZenUML's Cloudflare backend on its way to Confluence, and is not stored there. Otherwise, ZenUML's Cloudflare backend handles only app lifecycle events (install/upgrade), licence status checks, and anonymous usage telemetry.
...
Yes, once the diagram has been viewed at least once. Confluence's own page-level **PDF and Word export includes a snapshot of the diagram** as it appears on the page — the export captures a static image of the rendered macro, not the live interactive editor.

If a diagram was just created and nobody has opened the page to view it yet, that snapshot image may not exist yet — open the page once to generate it, then export again.
