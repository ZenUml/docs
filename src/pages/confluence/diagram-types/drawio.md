...
- **From the page viewer** — click the diagram to open the fullscreen viewer and use the Export button to download a **PNG** of the rendered diagram. Confluence's own page-level PDF/Word export also includes a snapshot of the diagram wherever it appears on a page.
- **From inside the DrawIO editor** — the diagram source is stored as **DrawIO XML**, which you can copy out via the editor's Extras menu. Open the exported XML in the standalone diagrams.net app for its full set of format options.
